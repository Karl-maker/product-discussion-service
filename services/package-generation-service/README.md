...
3. **State** – Load `lastProcessedAt` for `(userId, language)` from the generation-state table.
...
7. **Save** – Upsert the package (same `id` if updating) with `userId` and `language`; update `lastProcessedAt` for `(userId, language)`.
...
- **State table**: PK `USER#<userId>`, SK `LANGUAGE#<language>`, attribute `lastProcessedAt` (ISO string).
